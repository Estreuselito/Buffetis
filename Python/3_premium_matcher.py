...
sec_filings = pd.read_sql('SELECT CUSIP_8Digits as cusip2, MIN(date) AS date FROM Quarterly_investments csf GROUP BY cusip2', connection)
sec_filings['year'] = sec_filings['date'].astype(str).str[:4]
sec_filings['month'] = sec_filings['date'].astype(str).str[5:7]
...
final_df = pd.read_sql("select a.year, a.month, a.cusip, a.return, b.cusip2 from stock_match_mth a left join sec_filings_mth b on a.cusip = b.cusip2 and a.year = b.year and b.month = a.month", connection)
